            ...
            <div className='relative'>
                ...
                <div className='absolute inset-0 flex md:flex-row flex-col gap-y-20 text-2xl md:text-lg gap-x-6 items-center justify-center text-white font-bold tracking-wider'>
                    ...
                    <div ref={ref} className='text-center md:space-y-2 space-y-3'>
                        <h1>{inView && (<CountUp key={inView} start={0} end={15} duration={3} delay={0} />)}+</h1>
                        <h1>Years O<span className='border-t border-green pt-1 '>f Exp</span>erience</h1>
                    </div>
                    <div className='text-center md:space-y-2 space-y-3'>
                        <h1>{inView && (<CountUp key={inView} start={0} end={43} duration={3} delay={0} />)}+</h1>
                        <h1>Techno<span className='border-t border-green pt-1'>logy </span>Experts</h1>
                    </div>
                
                    <div className='text-center md:space-y-2 space-y-3'>
                        <h1>{inView && (<CountUp key={inView} start={0} end={1533} duration={3} delay={0} />)}+</h1>
                        <h1>Wholes<span className='border-t border-green pt-1'>ale P</span>artners</h1>
                    </div>
                    <div className='text-center md:space-y-2 space-y-3'>
                        <h1>{inView && (<CountUp key={inView} start={0} end={3892} duration={3} delay={0} />)}+</h1>
                        <h1>Satisfi<span className='border-t border-green pt-1'>ed Cu</span>stomers</h1>
                    </div>

                </div>
            </div>

            <div ref={ref} className='md:flex w-full pt-10 md:p-5 p-8'>
                <div className={`w-2/3 space-y-5 ${inView ? motionClassR : 'opacity-0'}`}>
                    <p className='text-grey font-medium text-sm'><span className='border-b border-green pb-1'>Our C</span>lients Know..</p>
                    <p className='text-blue font-extrabold text-2xl/9 w-90'>That we are the most reliable global supplier of <span className='text-green'> high-quality</span> IT equipment.</p>
                    <p className='text-grey font-semibold text-[9px] w-86'>We are dedicated to sourcing and providing high-quality, high-performance devices and partner with our clients to create value by ensuring their satisfaction and long-term success.</p>
                </div>
